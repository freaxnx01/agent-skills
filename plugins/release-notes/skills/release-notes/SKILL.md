---
name: release-notes
description: Generate user-friendly release notes in RELEASENOTES.md from git tags and commit history. Walks tags with `git tag --sort=-v:refname`, identifies versions missing from RELEASENOTES.md, collects commits per range with `git log <prev>..<tag> --no-merges`, and writes short plain-English summaries grouped by "New Features", "Improvements", "Bug Fixes". Use when the user mentions release notes, RELEASENOTES.md, "what's new", changelog for users, or runs /release-notes. Distinct from CHANGELOG.md (developer-facing, Conventional-Commit auto-gen) — this is the end-user companion. Never modifies existing entries; append-only for missing versions.
---
